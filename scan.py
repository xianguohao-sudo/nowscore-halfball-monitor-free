...
async def main():
    ...
    first_push_count = final_push_count = veto_count = 0
    changed = False
    ...
    for mid in sorted(ids):
        try:
            ...
            # 一票否决永远优先于“4/6”评分。即使分数够，也绝不推客队。
            if r.get("vetoed"):
                veto_count += 1
                for reason in r.get("vetoes", []):
                    print("否决:", reason)
                print("结果: VETO排除｜主队真实强化，不进入半球假强推送")
                continue
            ...
            if in_final_window:
                if match_state["final_sent"]:
                    ...
                    continue
                send_serverchan(f"临场确认 {r['score']}/6｜{m.home} vs {m.away}", message(m, r, "final", delta))
                now = datetime.now(TZ).strftime("%Y-%m-%d %H:%M:%S")
            ...
            if match_state["first_sent"]:
                print(f"结果: 符合｜首次提醒已推送，等待开赛前 {FINAL_WINDOW_MINUTES} 分钟内再做最终确认")
                continue

            send_serverchan(f"首次预警 {r['score']}/6｜{m.home} vs {m.away}", message(m, r, "first", delta))
            now = datetime.now(TZ).strftime("%Y-%m-%d %H:%M:%S")
            match_state["first_sent"] = True
            match_state["first_sent_at"] = now
            match_state["kickoff"] = m.kickoff
            ...
            pushed_count += 1
            first_push_count += 1
            print(f"结果: 符合 → 已推送【首次预警】；中间不重复，开赛前 {FINAL_WINDOW_MINUTES} 分钟内再确认一次")
